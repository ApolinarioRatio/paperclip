...
describe.sequential("issue comment reopen routes", () => {
  ...
  it("keeps closed issues terminal when a PATCH comment reassigns to an agent without reopen intent", async () => {
  ...
  it("keeps closed issues terminal via POST comments without reopen intent", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue("done"));
    mockIssueService.update.mockImplementation(async (_id: string, patch: Record<string, unknown>) => ({
      ...makeIssue("done"),
      ...patch,
  ...
  it("does not implicitly reopen done issues when the comment runId differs from the owning run", async () => {
    mockIssueService.getById.mockResolvedValue({
      ...makeIssue("done"),
